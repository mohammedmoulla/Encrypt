...
public class AES {
	...
	public static final String newline = System.getProperty("line.separator"); // The newline for whatever system you
																				// choose to run in.
	...
	static int keyFileIndex = 1; // Index where the keyFile argument should be. Used to determines the index of
									// other arguments.
...
//Nothing to initialize here.
...

	{
		 ...
		 * -mode => "ecb" or "cbc"
		 * 
		 * neither -length nor -mode: args[1] should be the keyFile, and args[2] should
		 * be the inputFile
		 *
		 * 
		 * 
...
//Defaults to 128-bit key size and ECB.
			...
			if (key.length() * 4 != keysizecheck) // Check to see if user's intended key size matches the size of key in
													// file.

			{

				throw new Exception(
						"Error: Attemping to use a " + key.length() * 4 + "-bit key with AES-" + keysizecheck);
...

		{
			...
			String line = "54 68 61 74 73 20 6D 79 20 4B 75 6E 67 20 46 75";
...
///////
			while (line != null) {
				...
				if (line.matches("[0-9A-F]+")) // If line is valid (i.e. contains valid hex characters, encrpyt.
												// Otherwise, skip line.

				{
					...
					aes.addRoundKey(state, aes.subKey(keymatrix, 0)); // Starts the addRoundKey with the first part of
																		// Key Expansion
...
//Helper method which executes a deep copy of a 2D array. (dest,src)
	...
	public int[][] subKey(int[][] km, int begin) {
	...
	/**
	 * 
	 * Inverse rendition of the subBytes. The operations of invSubBytes are the
	 * reverse operations of subBytes.
	 * 
	 ...
	 * 
	 * If the number of times to rotate the array is divisible by 4, return the
	 * array
	 * 
	 * as it is.
	 * 
	...
	/**
	 * 
	 * Helper method of mixColumns in which compute the mixColumn formula on each
	 * element.
	 * 
	 ...
	 * 
	 * @return the value in the corresponding mcX table based on the a,b
	 *         coordinates.
	 * 
	 */
	...
	/**
	 * 
	 * The keyScheduling algorithm to expand a short key into a number of separate
	 * round keys.
	 *
	 * 
	 * 
	...
	/**
	 * 
	 * For every (binary key size / 32)th column in the expanded key. We compute a
	 * special column
	 * 
	 * using sbox and an XOR of the an rcon number with the first element in the
	 * passed array.
	 * 
	 * 
	 * 
	 * @param in          the array in which we compute the next set of bytes for
	 *                    key expansion
	 * 
	 * @param rconpointer the element in the rcon array with which to XOR the first
	 *                    element in 'in'
	 * 
	 ...
	 * round, a chunk of the key scheduled is pulled; each subkey is the same size
	 * as the state. Each element in the byte matrix is XOR'd with each element in
	 * the chunk of the expanded key.
	 * 
	 * 
	 * 
	 * @param state     reference of the matrix in which addRoundKey will be
	 *                  computed upon.
	 * 
...
public String encrypt(String t,String k) {
	...
	aes.addRoundKey(state, aes.subKey(keymatrix, numRounds));
	String result="a#";
	result +=AES.MatrixToString(state,state.length,state[0].length);
	return result;
}
public static String decrypt(String t,String k) throws UnsupportedEncodingException {
	AES aes = new AES();
	String cipher=t.substring(2,t.length());
